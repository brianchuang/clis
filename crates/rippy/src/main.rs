    ...
    match action {
        ...
        HotkeyAction::Test => {
            ...
            if !hotkey::check_listen_permission(true) {
                eprintln!("Warning: Input Monitoring permission not granted. A system dialog should appear.");
                eprintln!();
            }
...
fn cmd_watch() -> Result {
    ...
    if running.load(Ordering::Relaxed) {
        eprintln!("Hotkey disabled: could not create event tap. Grant Input Monitoring permission to Rippy.");
